...
/*define O_INSTRUCTION_BASE(OP_NAME, MinWordCount, MaxWordCount, ParentStruct) \
...
instruction! { OpBranchConditional, 4, 65535,
    ...
    trueLabel: Id,          // Must be an OpLabel in the current function.
    falseLabel: Id,         // Must be an OpLabel in the current function.
    branchWeights: [LiteralNumber; 1]   // Branch weights are unsigned 32-bit integer literals. There must be either no Branch Weights or exactly two branch weights.
...
// This instruction must be the last instruction in a block.
    ...
    defaultLabel: Id,       // Must be the <id> of a label. If Selector does not equal any of the Target literals, control flow will branch to the Default label <id>.
...
// Capability: Kernel
    ...
    eventsList: Id        // Must be a pointer to OpTypeEvent.
}
...
// Capability: Kernel
    ...
    value: Id,              // Must be a 32 or 64 bits wise OpTypeInt or a 16, 32 or 64 OpTypeFloat
                                            // floating-point scalar datatype.
    localId: Id             // Must be an integer datatype. It can be a scalar, or a vector with 2 components or a vector
